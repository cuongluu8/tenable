...
**Status as of this commit: 33 of 70 batches loaded into production. 37 remain (this directory).**
...
Batches 000–032 have already been executed against production. This
directory contains the remaining, **not yet executed** batches: `batch_033.sql`
through `batch_069.sql`.

## Current DB state (verified via COUNT query at pause time)

- `reference_entities` totals: **9,305 players**, 488 clubs, 110 countries.
- Cloudflare D1 database: `database_id = a87ef250-cc94-4765-a821-785acbcd71a4`,
  name "tenable-content".

## Resume procedure (new session)

1. Read `db/pending_player_batches/batch_033.sql` (the next unexecuted batch —
...
session did 6 batches (027-032) so far, across two "carry on" turns.
