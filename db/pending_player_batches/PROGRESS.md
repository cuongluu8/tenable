...
**Status as of this commit: 34 of 70 batches loaded into production. 36 remain (this directory).**
...
Batches 000–033 have already been executed against production. This
directory contains the remaining, **not yet executed** batches: `batch_034.sql`
through `batch_069.sql`.

## Current DB state (verified via COUNT query at pause time)

- `reference_entities` totals: **9,555 players**, 488 clubs, 110 countries.
- Cloudflare D1 database: `database_id = a87ef250-cc94-4765-a821-785acbcd71a4`,
  name "tenable-content".

## Resume procedure (new session)

1. Read `db/pending_player_batches/batch_034.sql` (the next unexecuted batch —
...
session did 7 batches (027-033) so far, across two "carry on" turns.
