...
describe("remote session WebSockets", () => {
	...
	it("books the alarm for the next clock-driven change while a socket is open (the first hint tier), not a presence sweep", async () => {
		const host = await createSession("Host");
		...
		// the next poll noticed; now the alarm is booked for it. Nothing
		// earlier: presence is not swept on a timer (a lone player with
		// nobody waiting on them books no away check).
